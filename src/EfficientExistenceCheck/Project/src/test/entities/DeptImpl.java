...
public class DeptImpl extends EntityImpl {
  ...
  public static synchronized EntityDefImpl getDefinitionObject() {
    if (mDefinitionObject == null) {
      mDefinitionObject = (DeptDefImpl)EntityDefImpl.findDefObject("test.entities.Dept");
    }
  ...
  /**Validation method for Dname
   */
  public boolean validateDname(String  data) {
    DeptDefImpl entityDef = (DeptDefImpl)getDefinitionObject();
    String originalDnameValue = (String)getPostedAttribute(DNAME);
    /*
     * If Dname is being changed to a non-null value that is
     * different from the value we originally read-in from the database
     * then return true if no Dept already exists with the new Dname value.
     */
    if (data != null && !data.equalsIgnoreCase(originalDnameValue)) {
      return !(entityDef.existsByDname(getDBTransaction(),data));
    }
    return true;
  }
