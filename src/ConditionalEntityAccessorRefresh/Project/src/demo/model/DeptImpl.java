...
package demo.model;
import demo.fwkext.MyEntityImpl;
...
//  ---------------------------------------------------------------------
public class DeptImpl extends MyEntityImpl {
  ...
  /**
   * Returns false if some Emp instance in this department's associated
   * collection of emps has an Ename value of "XXX".
   */
  public boolean validateThatNoEmpHasDnameOfXXX() {
    RowIterator ri = getEmp();
    if (ri != null) {
      while (ri.hasNext()) {
        EmpImpl emp = (EmpImpl) ri.next();
        if ("XXX".equalsIgnoreCase(emp.getEname())) {
          return false;
        }
      }
    }
