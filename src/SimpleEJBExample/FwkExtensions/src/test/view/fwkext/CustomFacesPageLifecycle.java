...
import test.view.util.ADFUtils;
import test.view.util.EL;
import test.view.util.JSFUtils;
...
public class CustomFacesPageLifecycle extends FacesPageLifecycle {
  public void prepareModel(LifecycleContext ctx) {
    ...
    if (!(ctx.getBindingContext().getErrorHandler() instanceof 
          CustomDCErrorHandler)) {
      ctx.getBindingContext().setErrorHandler(new CustomDCErrorHandler(true));
    }
    ...
    if (numAttr > 0) {
      ...
      while (i.hasNext()) {
        ...
        /*
                   ...
                   */
        ControlBinding cb = ADFUtils.findControlBinding(bc, attrNameKey);
        if (cb != null) {
          String componentClientId = null;
          if (cb instanceof FacesCtrlAttrsBinding) {
            FacesCtrlAttrsBinding facesControlBinding = 
              (FacesCtrlAttrsBinding)cb;
          ...
          JSFUtils.addFacesErrorMessage(componentClientId, attrNameKey, msg);
        }
      }
    }
  }
  ...
  private void processException(Exception ex, List globalErrs, Map attrErrs, 
      ...
      if (obj != null) {
        ...
        if ((details == null) || (details.length == 0)) {
          ...
          if (ave.getErrorCode().equals(CSMessageBundle.EXC_VAL_ATTR_MANDATORY)) {
            ...
            localizedErrorMessage = ave.getLocalizedMessage(locale);
          }
        }
      }
    }
    ...
    if ((details != null) && (details.length > 0)) {
      for (int j = 0, count = details.length; j < count; j++) {
        processException((Exception)details[j], globalErrs, attrErrs, attrName, 
                         locale);
      }
    } else {
      /*
  ...
  protected void reportUnexpectedException(Exception ex) {
    List expectedExceptionPackages = (List)EL.get("#{AppInfo.expectedApplicationExceptionPackageNames}");
    if (expectedExceptionPackages != null 
        && !expectedExceptionPackages.contains(ex.getClass().getPackage().getName())) {
      ex.printStackTrace();
    }
  }
  ...
  public void validateModelUpdates(LifecycleContext lfContext) {
    forceValidationOfRowWithNullMandatoryAttributes(lfContext);
    super.validateModelUpdates(lfContext);
  }
  /*
     * If there are any attributes bindings for updateable, mandatory attributes
     * then set the first one found to dirty it to that validation is forced
     * to be performed even if the user didn't touch any fields.
     */
  private void forceValidationOfRowWithNullMandatoryAttributes(LifecycleContext lfContext) {
    ...
    for (JUCtrlAttrsBinding ab: attrBindings) {
      if (ab.isUpdateable() && ab.isMandatory()) {
        if (isNullOrEmpty(ab)) {
          dirtyNullBindingBySettingItNullAgain(ab);
          break;
        }
      }
    }
